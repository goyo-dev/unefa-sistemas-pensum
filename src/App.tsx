...
export default function App() {
  ...
  const [modalData, setModalData] = useState<{ isOpen: boolean; courseId: string | null; gradeStr: string }>({
    isOpen: false,
    courseId: null,
    gradeStr: '',
  });
  ...
  const resetProgress = () => {
    if (window.confirm("¿Estás seguro de que quieres reiniciar tu progreso y tus notas?")) {
      setPassedCourses({});
      setSelectedBlockedCourse(null);
    ...
    <div className={`min-h-screen font-sans flex flex-col transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      ...
      {modalData.isOpen && activeModalCourse && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          ...
          ></div>
          <div className={`relative w-full max-w-sm rounded-xl p-5 shadow-2xl border transition-colors duration-300 transform scale-100 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-800'}`}>
            <h3 className="text-base sm:text-lg font-bold tracking-tight mb-1">Registrar Calificación</h3>
            <p className={`text-xs mb-4 font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              Asignatura: <span className={isDarkMode ? 'text-blue-400' : 'text-blue-600'}>{activeModalCourse.name}</span> ({activeModalCourse.uc} UC)
            </p>
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2.5 mt-2">
                <button
                  onClick={() => setModalData({ isOpen: false, courseId: null, gradeStr: '' })}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-colors ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300' : 'bg-slate-100 hover:bg-slate-200 border-slate-200 text-slate-600'}`}
                >
                  Cancelar
                </button>
                <button
