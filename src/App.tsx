...
export default function App() {
  ...
  // --- SOLUCIÓN DEL BUG DE TYPESCRIPT: Ahora es un HTMLDivElement ---
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    dragDistance.current = Math.abs(e.clientX - startPos.current.x);
    ...
    >
      ...
      {showInstructions && (
        <div className="max-w-[1600px] mx-auto w-[calc(100%-2rem)] mt-4 p-4 rounded-xl border relative animate-fade-in text-xs sm:text-sm shadow-md bg-slate-100 border-slate-200 dark:bg-slate-900/50 dark:border-slate-800/80">
          ...
          <p className="text-[11px] text-slate-400 mt-2 text-center font-medium">
            🖱️ **Tip de Navegación en PC:** Mantén pulsado el clic izquierdo y
            arrastra a los lados para moverte fluidamente.
          </p>
        </div>
      )}
      ...
      <main className="flex-1 flex flex-col w-full relative overflow-hidden">
        <div
          ...
          className="w-full overflow-x-auto px-4 py-4 sm:px-6 sm:py-6 cursor-default active:cursor-grabbing [&::-webkit-scrollbar]:h-2 sm:[&::-webkit-scrollbar]:h-2.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300 dark:[&::-webkit-scrollbar-thumb]:bg-slate-700 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400 dark:hover:[&::-webkit-scrollbar-thumb]:bg-slate-600"
        >
          <div className="flex flex-row gap-4 sm:gap-6 min-w-max select-none pb-2 sm:pb-4">
                  ...
                  >
                    {courses.map((course) => {
                      ...
                      const isTargetBlocked =
                        selectedBlockedCourse === course.id;
      ...
      >
        <div className="max-w-[1600px] mx-auto flex flex-col justify-center items-center gap-2 lg:flex-row lg:justify-between select-none">
          ...
          <div
            className={`flex items-center justify-center gap-3 text-[10px] sm:text-xs px-3 py-1 rounded-full border w-full sm:max-w-md ${
              isDarkMode
