...
export default function App() {
  ...
  // --- VARIABLES DE ARRASTRE HORIZONTAL LIMPIAS ---
  ...
  // --- LÓGICA DE ARRASTRE HORIZONTAL EXCLUSIVO ---
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const walkX = (e.clientX - startPos.current.x) * 1.5; 
    dragDistance.current = Math.abs(e.clientX - startPos.current.x) + Math.abs(e.clientY - startPos.current.y);
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      ...
      {showInstructions && (
        <div className="max-w-[1600px] mx-auto w-[calc(100%-2rem)] mt-4 p-4 rounded-xl border relative animate-fade-in text-xs sm:text-sm shadow-md bg-slate-100 border-slate-200 dark:bg-slate-900/50 dark:border-slate-800/80">
          ...
          <p className="text-[11px] text-slate-400 mt-2 text-center font-medium">
            🖱️ **Tip de Navegación en PC:** Mantén pulsado el clic izquierdo y arrastra a los lados para moverte fluidamente.
          </p>
        </div>
      )}
      ...
      {/* Grid de Semestres (Retorno a overflow-x-auto puro) */}
      <main 
      ...
      <footer className={`border-t p-2 sm:p-4 sticky bottom-0 z-20 shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] shrink-0 transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        <div className="max-w-[1600px] mx-auto flex flex-col justify-center items-center gap-2 lg:flex-row lg:justify-between select-none">
          ...
          <div className={`flex items-center justify-center gap-3 text-[10px] sm:text-xs px-3 py-1 rounded-full border w-full sm:max-w-md ${isDarkMode ? 'bg-slate-800 text-slate-500 border-slate-700' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
